...
def init():
    ...
    departments = make_department(sheets)
    num_faculty = get_all_faculty_count(departments)
...
"""
MODULE ONE: CONVERTING DATA
Takes an excel sheet and convert data into Python objects.
"""


def make_department(sheets):
    """"Returns a list of departments (Department objects)"""

    # status
    print("\n" + "Making Departments...")

    # list that stores departments
    departments = []
    ...
    for sheet in sheets:
        ...
        people = make_person(sheet)

        new_department = Department(title, people)
...
def make_person(sheet):
    """Returns a list of faculty members (Person objects)"""
...
def get_all_faculty_count(departments):
    """Returns an integer: the number of all faculty"""

    faculty_count = 0

    for department in departments:
        faculty_count += len(department.people)

    return faculty_count


"""
MODULE TWO: VERIFYING DATA
Contains multiple functions that take in departments and 
checks whether an attribute of a Person object is valid.
"""
